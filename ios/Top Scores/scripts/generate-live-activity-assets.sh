...
generate_variant() {
  ...
  local source_contents="$ASSET_CATALOG/$base_name.imageset/Contents.json"

  if [[ ! -f "$source_contents" ]]; then
    if [[ "$required" == "true" ]]; then
      echo "Missing source asset: $base_name" >&2
      return 1
    fi
    echo "Skipping manifest entry without an asset: $base_name" >&2
    return 2
  fi

  local filename source_path variant_name variant_dir output_filename
  filename="$(source_filename "$source_contents")"
  source_path="$ASSET_CATALOG/$base_name.imageset/$filename"
...
while IFS= read -r team_name; do
  generate_variant "$team_name" 36 36 false || status=$?
