...
enum PerformanceSignposter {
    ...
    private static func makeSignposter(
        category: String,
        enabledInDeployedBuilds: Bool = false
    ) -> OSSignposter {
        #if DEBUG
        OSSignposter(
        ...
        #else
        enabledInDeployedBuilds
            ? OSSignposter(logger: Logger(subsystem: subsystem, category: category))
            : OSSignposter.disabled
        #endif
    }

    nonisolated static let startup = makeSignposter(category: "performance.startup")
    nonisolated static let matches = makeSignposter(
        category: "performance.matches",
        enabledInDeployedBuilds: true
    )
